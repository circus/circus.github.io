...
<html doctype>
	...
	<body>
		...
		<h1>
			...
			<br><span class="red">Case</span> <code>W05-2</code>
		</h1>
		<dl><dt>Source</dt>
...
<dt>Views</dt>
	<dd>Higher-level protocol state machine A</dd>
	<dd>lower-level protocol state machine C</dd>
<dt>Artefacts</dt>
	<dd>open/close/read/write protocol states and transitions</dd>
<dt>Quotes</dt>
	<dd>"The state machine of A belongs to a higher-level model, it is nondeterministic and also models the case where the file to be opened is non-existent... In C the nondeterminism has been resolved, possibly by ensuring applicability of open on existing files only."</dd>
<dt>Summary</dt>
	<dd>A lower-level protocol removes the higher-level nondeterministic case in which open may target a non-existent file.</dd>
	<dd>Cause: protocol refinement tightens behavioral assumptions relative to the abstract view.</dd>
	<dd>As a result, behavioral possibilities differ across abstraction levels; refinement must be checked to ensure the lower-level behavior is still permitted by the higher-level one.</dd>
